package com.park61.widget.wheel.weekcalendar;
...
import com.park61.widget.wheel.weekcalendar.listener.OnDateClickListener;
import com.park61.widget.wheel.weekcalendar.view.WeekPager;
...
public class WeekCalendar extends LinearLayout {
	...
	/***
	 * Do not use this method this is for receiving date, use "setOndateClick"
	 * instead.
	 */
	@Subscribe
	...
	private GridView getDaysNames() {
		...
		daysName.setAdapter(new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if (typedArray != null) {
					day.setTextColor(typedArray.getColor(R.styleable.WeekCalendar_weekTextColor, Color.BLACK));
					day.setTextSize(TypedValue.COMPLEX_UNIT_PX,
							typedArray.getDimension(R.styleable.WeekCalendar_weekTextSize, day.getTextSize()));
				}
				return convertView;
			}
			...
			private String[] getWeekDayNames() {
				...
				if (typedArray.getInt(R.styleable.WeekCalendar_dayNameLength, 0) == 0)
					for (int i = 0; i < daysName.size(); i++)
						daysName.set(i, daysName.get(i).replace("周", ""));
		...
		if (typedArray != null)
			daysName.setBackgroundColor(typedArray.getColor(R.styleable.WeekCalendar_weekBackgroundColor,
