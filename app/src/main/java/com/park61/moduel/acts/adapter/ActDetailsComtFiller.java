...
public class ActDetailsComtFiller {
	...
	public static void buildListInLinear(LinearLayout linear_comment,
			List<ComtItem> comtList, Context mContext,
			final OnReplyClickedLsner lsner) {
		linear_comment.removeAllViews();
		for (int i = 0; i < comtList.size(); i++) {
			final ComtItem ci = comtList.get(i);
			View convertView = LayoutInflater.from(mContext).inflate(
					R.layout.comtlist_item, null);
			...
			MyRatingBar ratingbar_score = (MyRatingBar) convertView
			...
			fillEvaImages(lv_eva_imgs, ci.getListPictrue(), mContext);
			ImageManager.getInstance().displayImg(img_user, ci.getUserHead());
			tv_username.setText(ci.getUserName());
			tv_comt_content.setText(ci.getContent());
			tv_comt_time.setText(DateTool.L2S(ci.getCreateTime()));
			if (ci.getReplyList() != null && ci.getReplyList().size() > 0) {
				linear_reply.setVisibility(View.VISIBLE);
				buildReplyList(linear_reply, ci, mContext, lsner);
			} else {
			...
			if (ci.getType() == 0) {// 评价显示星星
				ratingbar_score.setVisibility(View.VISIBLE);
//				ratingbar_score.setPoint(ci.getCompositeScore().intValue());
				ratingbar_score.setPoint(ci.getActivityTotalScore().intValue());
			} else {// 评论不显示星星
				ratingbar_score.setVisibility(View.GONE);
			}

			tv_comt_content.setOnClickListener(new OnClickListener() {

				@Override
				public void onClick(View v) {
					if (ci.getType() == 1)// 评论可回复，评价不可回复
						lsner.onComtClicked(ci.getId());
				}
			});
	...
	public static void fillEvaImages(LinearLayout lv_eva_imgs,
			List<EvaImage> listPictrue, final Context mContext) {
		...
		for (int i = 0; i < imgViewList.size(); i++) {
			ImageManager.getInstance().displayImg(imgViewList.get(i), listPictrue.get(i).getPictureUrl());
		}
	...
	public static void buildReplyList(LinearLayout lv, final ComtItem ci,
		...
		for (int i = 0; i < replyList.size(); i++) {
			...
			convertView.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					lsner.onReplyClicked(ci.getId(), ri.getId());
				}
			});
