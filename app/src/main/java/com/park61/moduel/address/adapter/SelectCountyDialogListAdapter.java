 ...
 *
 */
public class SelectCountyDialogListAdapter extends BaseAdapter {

	private List<CityBean> lists;
	private LayoutInflater factory;
	private int selectPosition=0 ;	

	public SelectCountyDialogListAdapter(List<CityBean> lists, Context mContext) {
		super();
		this.lists = lists;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.tv_select_address.setText(
					TextUtils.isEmpty(lists.get(position).getCountyName()) ? lists.get(position).getName() : lists.get(position).getCountyName());
