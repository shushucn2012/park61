...
public class SalesPageBanner2HeaderDelegate implements ItemViewDelegate<GoodsCombine> {

    private Context mContext;
    private MyBanner mp;

    public SalesPageBanner2HeaderDelegate(Context c) {
        mContext = c;
    }

    @Override
    public int getItemViewLayoutId() {
        return R.layout.top_banner2_layout;
    }

    @Override
    public boolean isForViewType(GoodsCombine item, int position) {
        return item.getTemplete().getTempleteCode().equals("banner_two");
    }
