...
public abstract class PullToZoomBase<T extends View> extends LinearLayout implements IPullToZoom<T> {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            //初始化状态View
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.PullBezierZoomView);

            int zoomViewResId = a.getResourceId(R.styleable.PullBezierZoomView_zoomView, 0);
            if (zoomViewResId > 0) {
                mZoomView = mLayoutInflater.inflate(zoomViewResId, null, false);
            }
            int headerViewResId = a.getResourceId(R.styleable.PullBezierZoomView_headerView, 0);
            if (headerViewResId > 0) {
                mHeaderView = mLayoutInflater.inflate(headerViewResId, null, false);
            }
            isParallax = a.getBoolean(R.styleable.PullBezierZoomView_isHeaderParallax, true);
            isBezier = a.getBoolean(R.styleable.PullBezierZoomView_isBezier, false);
            int color = a.getColor(R.styleable.PullBezierZoomView_bezierColor, 0xFFFFFFFF);
            float maxHeight = a.getDimension(R.styleable.PullBezierZoomView_maxBezierHeight, 0);
            if (isBezier) {
                mBezierView = new BezierView(getContext());
                mBezierView.setColor(color);
                if (maxHeight > 0) {
                    mBezierView.setMaxHeight(maxHeight);
                }
            }
    ...
    public abstract void setHeaderView(BezierView headerView);
