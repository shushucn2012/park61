 ...
 *
 */
public class LogisticsDetailListAdapter extends BaseAdapter {
	private List<OrderTrack> mList;
	private Context mContext;
	private LayoutInflater factory;
	
	public LogisticsDetailListAdapter(List<OrderTrack> mList,Context context){
		this.mList = mList;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		OrderTrack item = mList.get(position);
		holder.tv_detail.setText(item.getProcessContent());
		holder.tv_time.setText(DateTool.L2S(item.getProcessCreateTime()));
		if(mList.size()<=1){
